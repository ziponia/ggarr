 ...
 */
public interface UtilComponent extends Serializable {
    ...
    /**
     * MultipartFile 타입을 File 로 변환한다.
     *
     * @param multipartFile sourceFile
     * @return java.io.File
     */
    static File multipartToFile(MultipartFile multipartFile) throws IOException {
        File convFile = new File(Objects.requireNonNull(multipartFile.getOriginalFilename()));
        boolean createFile = convFile.createNewFile();
     ...
     * @return java.lang.String
     */
    static String orderNumberGenerator() {
     ...
     * @return boolean
     */
    static boolean checkLicenseValidity(String licenseNo) {
     ...
     * @return boolean
     */
    static boolean checkCellphone(String str) {
    ...
    static byte[] encryptFile(Key key, byte[] content) {
        Cipher cipher;
        byte[] encrypted = null;
        try {
    ...
    static byte[] decryptFile(Key key, byte[] textCryp) {
        Cipher cipher;
        byte[] decrypted = null;
        try {
            cipher = Cipher.getInstance("AES/ECB/PKCS5Padding");
     ...
     * @return boolean
     */
    static boolean passwordThen8AndSpecialCharacters(String password) {
